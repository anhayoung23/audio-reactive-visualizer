...
소리는 눈에 보이지 않지만 끊임없이 흐르고 변화합니다. Resonant Flow는 실시간 오디오 입력을 분석해, 그 흐름을 3D 지형(terrain) 형태의 시각적 구조로 옮기는 작업입니다. 진폭과 주파수의 변화가 곧 형태의 높낮이와 색으로 번역되며, 소리가 멈추지 않는 한 지형도 고정되지 않고 계속 흐르며 변화합니다.

## Concept

청각적 경험은 본래 형태가 없지만, 우리는 늘 그것을 듣고 기억합니다. Resonant Flow는 그 감각의 번역 : 들리는 것을 보이는 것으로, 순간을 지속되는 형태로 바꾸는 비정형 시각화 결과물입니다. 소리의 주파수에 따라 매 순간 다시 그려지는 것처럼 이 결과물은 고정된 것이 아니라 소리와 함께 계속 생성되는 하나의 흐름으로 완성되고, 재구성됩니다.
...
Audio Spectrum CHOP에서 나온 데이터 개수(2048개)를, 지형을 이루는 점의 개수에 맞게 다시 조정해주는 노드
